...
void dtr0xx_ioComponent::setup() {
  ...
  if (this->dingtian_rck_pin_ != nullptr) {
    ...
    // Keep the storage latch low until the all-off buffer has been shifted.
    // The first low-to-high edge at the end of read_gpio_() then publishes
    // known zeros instead of unknown power-on shift-register contents.
...
void dtr0xx_ioComponent::loop() {
  // This sprinkler controller only uses the Dingtian shift register as relay
  // outputs. The upstream component clocks the register on every ESPHome loop
  // to keep input bits fresh, but that is unnecessary here and adds constant
  // GPIO/latch activity while valves are open. Outputs are still shifted when
  // digital_write_() changes a relay state.
}
