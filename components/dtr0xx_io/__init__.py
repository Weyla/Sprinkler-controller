...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_SR_COUNT, default=1): cv.int_range(min=1, max=255),
    }
...
def validate_mode(value):
    if value.get(CONF_INPUT, False) or not value.get(CONF_OUTPUT, False):
        raise cv.Invalid("This controller fork supports output pins only")
    return value


dtr0xx_io_PIN_SCHEMA = pins.gpio_base_schema(
    dtr0xx_ioGPIOPin,
    cv.int_range(min=0, max=2039),
    modes=[CONF_OUTPUT],
