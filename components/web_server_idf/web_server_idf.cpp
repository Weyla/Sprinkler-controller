...
#include "utils.h"
// Modified for Sprinkler Controller on 2026-07-12: receive fragmented form
// bodies until the declared Content-Length is complete.
...
esp_err_t AsyncWebServer::request_post_handler(httpd_req_t *r) {
  ...
  if (r->content_len > 0) {
    ...
    while (received < r->content_len) {
      const int ret = httpd_req_recv(r, post_query.data() + received, r->content_len - received);
